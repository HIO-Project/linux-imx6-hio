...
int download_patch(struct usb_interface* intf)
{
	...
	//check fw version, if different,do not need download fw
	ret_val = check_fw_version(xdata);
	if (ret_val != 0)
	{
		//if global_version is not availble, get it!
		if(global_eversion == 0xFF) {
			RTKBT_DBG("global_version is not availble, get it!");
			rtk_get_eversion(dev_entry);
		}
		goto patch_end;
	}
	...
	//check fw version again, check download fw success!
	ret_val = check_fw_version(xdata);
	if (ret_val <= 0)
	{
		ret_val = -1;
...
uint8_t rtk_get_eversion(dev_data * dev_entry)
{
	...
	RTKBT_DBG("rtk_get_eversion::global_eversion=%d", global_eversion);
	if(global_eversion != 0xFF) {
		RTKBT_DBG("global_eversion != 0xFF, return it directly!");
